...
public class PoiSvriceImpl {
	 ...
	 * @throws IOException 
	 */
	public void addProductDetailPoi(Store getStore,ServletOutputStream out,Date startDate,Date endDate) throws IOException {
		
		List<OrderDetail> getListOrderDetail = poiDaoImpl.findOrderToPOI(getStore.getStoreId(),startDate,endDate);
		try{
			//1.建立工作簿
	...
	public void addFundProductDetailPoi(Store getStore,ServletOutputStream out,Date startDate,Date endDate) throws IOException {
		...
		try{
			...
			HSSFCell cell3 = row3.createCell(2);
			cell3.setCellValue(getListFundOrderDetail.get(j).getFundOrderProductName());
			HSSFCell cell4 = row3.createCell(3);
