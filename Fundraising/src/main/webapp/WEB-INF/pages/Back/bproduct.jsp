		...
		<div class="row">
			...
			<main role="main" class="col-md-9 ml-sm-auto col-lg-10 px-4">
				...
				<c:if test="${empty Inquire}">
					<!-- 如果 沒有輸入值 -->
					<form action="productLike">
						<h3>請輸入查詢條件( 可為productId、模糊查詢(貨號、名稱) )</h3>
					...
					<table class="table table-striped table-sm">
						...
						<tbody>
							<c:forEach items="${all }" var="all">
								<tr>
									<td>${all.productId}</td>
									<td>${all.productItemNO}</td>
									<td>${all.productName}</td>
									<td>${all.productPrice}</td>
									<td><img alt="" src="${all.productPic}" width="50px"
										height="50px"></td>
									<td>${all.productStatus}</td>
									<td>${all.store.storeId}</td>
							</c:forEach>
							<c:forEach items="${listProduct}" var="listProduct">
								<tr>
									<td><c:out value="${listProduct.productId}" /></td>
									<td><c:out value="${listProduct.productItemNO}" /></td>
									<td><c:out value="${listProduct.productName}" /></td>
									<td><c:out value="${listProduct.productPrice}" /></td>
									<td><img alt="" src="${listProduct.productPic}"
										width="50px" height="50px"></td>
									<td>${listProduct.productStatus}</td>
									<td>${listProduct.store.storeId}</td>
