		...
		<div class="row">
			...
			<main role="main" class="col-md-9 ml-sm-auto col-lg-10 px-4">
				...
				<c:if test="${empty Inquire}">
					<!-- 如果 沒有輸入值 -->
					<form action="storeLike">
						<h3>請輸入查詢條件( 可為storeId、模糊查詢(姓名、身分證字號) )</h3>
						<select name="InquireSelect">
							<option value="1">storeId</option>
					...
					<table class="table table-striped table-sm">
						...
						<tbody>
							<c:forEach items="${all }" var="all" varStatus="st">
								<form action="getUpdateBStoreStatus" method="get">

									<input type="hidden" value="${all.storeId}" name="storeId">
									<tr>
										<td>${all.storeId}</td>
										<td>${all.storeName}</td>
										<td>${all.storePf}</td>
										<td>${all.storeTel}</td>
										<td><img alt="" src="${all.storePic}"
											style="height: 50px; width: 50px"></td>
										<td>${all.storeAddress}</td>
										<td>${all.storeNumber}</td>
										<td>${all.storeAccount}</td>
										<td>${all.storePiName}</td>
										<td>${all.storePiTwid}</td>
										<td>${all.storePiTel}</td>
										<td>${all.storePiAddress}</td>
										<td>${all.storeStatus}</td>
						...
						<tbody>
							...
							<c:forEach items="${listStore}" var="listStore">
								<form action="getUpdateBStoreStatus" method="get">
									<input type="hidden" value="${listStore.storeId}"
										name="storeId">
								<tr>
									<td>${listStore.storeId}</td>
									<td>${listStore.storeName}</td>
									<td>${listStore.storePf}</td>
									<td>${listStore.storeTel}</td>
									<td><img alt="" src="${listStore.storePic}"
										style="height: 50px; width: 50px"></td>
									<td>${listStore.storeAddress}</td>
									<td>${listStore.storeNumber}</td>
									<td>${listStore.storeAccount}</td>
									<td>${listStore.storePiName}</td>
									<td>${listStore.storePiTwid}</td>
									<td>${listStore.storePiTel}</td>
									<td>${listStore.storePiAddress}</td>
									<td>${listStore.storeStatus}</td>
									<td><input type="submit" class="btn btn-outline-primary"
										value="更改"></td>
								</tr>
								</form>
